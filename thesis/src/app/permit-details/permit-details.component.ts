...
export class PermitDetailsComponent implements OnInit {
  ...
  public currentPermit = new Permit(this.professorId, null, null, null, null);
  ...
  ngOnInit(): void {
    ...
    this.permitModel = new Permit(this.professorId, null, null, null, null);

    this._professorsService.getProfessorById(this.professorId)
      .subscribe(data => this.permit_list = Object.entries(data)[13][1],
  ...
  onSubmit(form: NgForm) {
    ...
    if (this.formState == 'newPermit') {
      this.permitModel = new Permit(this.professorId, this.permitModel.title, this.permitModel.description, this.permitModel.from, this.permitModel.until);
